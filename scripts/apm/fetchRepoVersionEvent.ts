...
const repoAbi = ["event NewVersion(uint256 versionId, uint16[3] semanticVersion)"];
...
export async function getRegistryOnRange(
  ...
  const registryInterface = new ethers.utils.Interface(repoAbi);
  const eventNewVersionTopic = registryInterface.getEventTopic("NewVersion");
  ...
  let latestBlock = _highestBlock;
  let blockStep = maxBlocksPerRequest;

  while (events.length < versionCount) {
    const from = latestBlock;
    const to = Math.min(latestBlock + blockStep, _highestBlock);
